...
export class DrawingApp {
  ...
  tick() {
    ...
    if (points.length >= 2) {
      ...
      if (this.self.tool === 'flowPen' && this.self.mousedown && !this.self.panning) {
        ...
        const tool = this.toolManager.getCurrentTool();
        broadcastPoints = tool.drainStampBuffer();
      } else if (this.self.mousedown && !this.self.panning) {
  ...
  processSmoothingCatchup() {
    ...
    if (this.self.tool === 'flowPen') {
      const stamps = tool.drainStampBuffer();
      if (stamps.length > 0) {
        this.wsClient.broadcastMove(stamps);
      }
    } else {
