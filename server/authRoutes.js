...
export async function handlePasswordResetRequest(req, res) {
  ...
  const secretAnswer = typeof body.secretAnswer === 'string' ? body.secretAnswer.trim() : '';
  if (!identifier) return json(res, 400, { success: false, error: 'Email or username required' });

  try {
    const isEmail = identifier.includes('@');
    const user = await db.collection('users').findOne(
      isEmail ? { email: normalizeEmail(identifier) } : { username: normalizeUsername(identifier) },
      {
        collation: { locale: 'en', strength: 2 },
    ...
    if (!user) return json(res, 200, { success: true, message: genericMessage });

    if (isEmail || (!secretAnswer && user.email && !(user.secretQuestion && user.secretAnswerHash))) {
      const token = await createPasswordResetToken(db, user, req);
      const emailSent = await sendPasswordResetEmail({
        to: user.email,
        username: user.username,
        resetLink: buildResetLink(req, token),
      });
...
export async function handlePasswordResetComplete(req, res) {
  ...
  const password = typeof body.password === 'string' ? body.password : '';
  if (!token || !password) return json(res, 400, { success: false, error: 'Reset token and new password required' });
  if (password.length < 6) return json(res, 400, { success: false, error: 'Password must be at least 6 characters' });
  ...
  try {
    ...
    if (!resetDoc) return json(res, 400, { success: false, error: 'Reset link is invalid or expired' });
