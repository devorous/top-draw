...
const broadcastLatency = new Trend('broadcast_latency_fast');
...
export default function () {
  ...
  const res = ws.connect(url, {}, function (socket) {
    socket.on('open', function () {
      socket.sendBinary(buildMsg({ t: T.CONNECT, n: `FAST_VU_${__VU}` }));
      ...
      // Randomize initial state and cycle length to prevent synchronicity
      ...
      socket.setInterval(function() {
        ...
        if (state === 0) {
          // No teleporting - just start from current x,y
          ...
          if (stateTicks >= nextPropertyChange) {
            ...
            // Hardness capped at 67%
            socket.sendBinary(buildMsg({ t: T.CHD, u: sessionIndex, hd: Math.floor(Math.random() * 6701) }));
            socket.sendBinary(buildMsg({ t: T.CS, u: sessionIndex, s: Math.floor(Math.random() * 2000) + 500 }));
            
            nextPropertyChange = stateTicks + (Math.floor(Math.random() * 4) + 2) * cycleLength;
          }
        } 
        else if (state === 1) {
          ...
          // Continuous movement with jitter and bounce
        ...
        if (state > cycleLength) {
           state = 0;
           // Occasionally change cycle length to break patterns
