...
export default function () {
  ...
  const res = ws.connect(url, {}, function (socket) {
    socket.on('open', function () {
      ...
      // across the whole board. Uses the seeded rng in the free-roam path so
      // --seed still reproduces a run.
      ...
      if (CLUSTERS > 0) {
        ...
        homeX = rng() * (BOARD_WIDTH - REGION_SIZE - 2 * margin) + margin + REGION_SIZE / 2;
        homeY = rng() * (BOARD_HEIGHT - REGION_SIZE - 2 * margin) + margin + REGION_SIZE / 2;
      }
