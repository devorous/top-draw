/** @fileoverview K6 stress test for low-volume WebSocket traffic with realistic user behavior. */
...
const BEHAVIOR_PROFILES = {
  ARTIST: { // Draws mostly with brush/pen, occasional text
    ...
    strokeLength: [60, 120], // longer strokes
    strokeCount: [3, 7],
  },
...
export default function () {
  ...
  const res = ws.connect(url, {}, function (socket) {
    socket.on('open', function () {
      ...
      // Define a localized "home area" for this user (400x400 region)
      ...
      let state = 0; // 0=idle, 1=ready, 2=drawing, 3=typing
      let stateTicks = 0;
      let cycleLength = 0;
      ...
      socket.setInterval(function() {
        ...
        if (state === 0) { // IDLE - pick what to do next
          ...
          // Stay idle for a bit (simulate thinking/looking)
          if (idleTicks < 15 + Math.floor(Math.random() * 30)) {
            return;
          }
          ...
          const size = 500 + Math.floor(Math.random() * 2000); // 5-25px
