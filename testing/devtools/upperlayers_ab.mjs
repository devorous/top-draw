#!/usr/bin/env node
/**
 * @fileoverview Paired A/B for Board._compositeUpperLayers dirty-rect handling.
 *
 * Sequential board_perf_suite runs could not settle this: the eraser arm read
 * 40.0 -> 37.1 -> 34.3 fps across three runs while the brush control stayed at
 * ~51 fps and the CPU sat at 54 C, so there is session drift that is larger than
 * the effect being measured and it is not thermal. Interleaving the two variants
 * inside ONE page session cancels that drift — the same trap [[observer_bake_order
 * _divergence]] describes for bake ordering: never judge on one run per arm.
 *
 * OLD is the shipped-until-now behaviour (hardcoded null dirtyRects => full-board
 * clear + full-board re-composite of the upper layers, every frame). NEW is the
 * current source. Both are installed on the live Board instance and alternated.
 *
 * Usage:
 *   CDP_URL=http://127.0.0.1:9222 node testing/devtools/upperlayers_ab.mjs --reps=4
 */
...
const SETUP = `(() => {
  ...
  const board = app.board;
  const NEW = board._compositeUpperLayers.bind(board);

  // The shipped-until-now implementation, verbatim.
  const OLD = function (startIdx, endIdx, dirtyRects) {
    if (!board.upperLayersCtx) return;
    board.layerManager.compositeLayerRange(board.upperLayersCtx, startIdx, endIdx, null, null);
    board._upperLayersCompositeStart = startIdx;
    board._setLayerPresent(board.upperLayersCanvas, endIdx > startIdx);
  };

  window.__setVariant = (which) => {
    board._compositeUpperLayers = which === 'old' ? OLD : NEW;
    // Both variants key off this state; reset so neither inherits the other's
    // idea of what the canvas already holds.
...
(async () => {
  ...
  try {
    ...
    const room = `ab_${Date.now()}`;
    await page.evaluate((r) => { window.app.self.username = 'AB'; window.app.handleRoomSelected(r); }, room);
    ...
    // Untimed warm-up so JIT and first-touch allocation are not charged to arm A.
    await page.evaluate(() => window.__setVariant('new'));
    await page.evaluate(() => window.__drive({ strokes: 2, pts: 20 }));

    const runs = { old: [], new: [] };
    console.log(`\n=== upperlayers A/B  tool=${TOOL}  ${SIZE}  ${REPS} reps x ${STROKES} strokes, interleaved\n`);
    console.log('  rep  variant     fps     p50     p99   >20ms   >50ms');
    for (let i = 0; i < REPS; i++) {
      ...
      for (const variant of order) {
        ...
        runs[variant].push(r);
        console.log(`  ${String(i + 1).padStart(3)}  ${variant.padEnd(8)}`
    ...
    console.log('\n  medians:');
    console.log('  variant     fps     p50     p99   >20ms   >50ms');
