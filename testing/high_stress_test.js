/**
 * @fileoverview K6 stress test for high-volume WebSocket traffic. Each VU
...
const STROKE_LENGTH = [20, 110];
const STROKE_COUNT  = [3, 10];
  ...
  const res = ws.connect(url, {}, function (socket) {
    socket.on('open', function () {
      ...
      socket.setInterval(function () {
        ...
        if (state === 0) {
          idleTicks++;
          if (idleTicks < 5 + Math.floor(Math.random() * 15)) return;
          idleTicks = 0;
